...
public class GluttonEntity extends PathfinderMob implements IAnimatable {
    ...
    protected void registerGoals() {
        ...
        this.targetSelector.addGoal(2, new NearestAttackableTargetGoal<>(this, GluttonEntity.class, true));
        this.targetSelector.addGoal(3, new NearestAttackableTargetGoal<>(this, LivingEntity.class, 10, true, false, (entity) -> {
            return this.distanceToSqr(entity) < 2 && !(entity instanceof Creeper);
        }));
    }
    ...
    public static boolean canSpawn(EntityType<GluttonEntity> entity, LevelAccessor world, MobSpawnType spawnReason, BlockPos blockPos, Random random) {
        return world.getRawBrightness(blockPos, 0) > 8 && checkMobSpawnRules(entity, world, spawnReason, blockPos, random) && world.getBlockState(blockPos.below()).is(ModTags.Blocks.GLUTTON_CAN_BURROW);
    }
    ...
    static {
        BURROWED = SynchedEntityData.defineId(GluttonEntity.class, EntityDataSerializers.BOOLEAN);
        ATTACKING = SynchedEntityData.defineId(GluttonEntity.class, EntityDataSerializers.BOOLEAN);
        ENCHANTING = SynchedEntityData.defineId(GluttonEntity.class, EntityDataSerializers.BOOLEAN);
        VARIANT = SynchedEntityData.defineId(GluttonEntity.class, EntityDataSerializers.INT);
    }
        ...
        for (double k = posX - 1; k <= posX + 1; ++k) {
            if (ret) {
                for (double l = posZ - 1; l <= posZ + 1; ++l) {
                    ...
                    if (block.is(ModTags.Blocks.GLUTTON_CAN_BURROW) && !gluttonIn.isInWater()) {
                        if (ret) {
