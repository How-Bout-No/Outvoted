    ...
    public static void spawnEntities(BiomeLoadingEvent event) {
        ...
        if (OutvotedConfig.COMMON.spawnwildfire.get()) {
            if (event.getCategory() == Biome.Category.NETHER) {
                if (!OutvotedConfig.COMMON.restrictwildfire.get() || biomename.equals("minecraft:nether_wastes")) {
                    event.getSpawns().addSpawn(EntityClassification.MONSTER, new MobSpawnInfo.Spawners(ModEntityTypes.WILDFIRE.get(), OutvotedConfig.COMMON.ratewildfire.get(), 1, 1));
                }
            }
        }
        if (OutvotedConfig.COMMON.spawnhunger.get()) {
            if (event.getCategory() == Biome.Category.DESERT || event.getCategory() == Biome.Category.SWAMP) {
                event.getSpawns().addSpawn(EntityClassification.MONSTER, new MobSpawnInfo.Spawners(ModEntityTypes.HUNGER.get(), OutvotedConfig.COMMON.ratehunger.get(), 1, 1));
            }
        }
        if (OutvotedConfig.COMMON.spawnkraken.get()) {
            ...
            if (event.getDepth() == -1.8F && !biomename.equals("minecraft:deep_frozen_ocean")) {
                event.getSpawns().addSpawn(EntityClassification.WATER_CREATURE, new MobSpawnInfo.Spawners(ModEntityTypes.KRAKEN.get(), OutvotedConfig.COMMON.ratekraken.get(), 1, 1));
            }
        }
    }
        ...
        if (OutvotedConfig.COMMON.spawnkraken.get() || OutvotedConfig.COMMON.spawnhunger.get()) {
            if (e instanceof KrakenEntity || e instanceof HungerEntity) {
                if (event.getSpawnReason() == SpawnReason.NATURAL) {
                    List<Entity> entities = event.getWorld().getEntities(event.getEntity(), event.getEntity().getBoundingBox().expandTowards(area, area, area).expandTowards(-area, -area, -area));
                    if (!entities.isEmpty()) {
            ...
            if (e instanceof WildfireEntity) {
                if (event.getSpawnReason() == SpawnReason.NATURAL) {
                    ...
                    for (int i = 1; i <= rand; i++) {
                        ...
                        blaze.absMoveTo(e.getRandomX(2.0D), e.getY(), e.getRandomZ(2.0D), e.yRot, e.xRot);
                        while (!world.isEmptyBlock(blaze.blockPosition())) { // Should prevent spawning inside of blocks
                            blaze.absMoveTo(e.getRandomX(2.0D), e.getY(), e.getRandomZ(2.0D), e.yRot, e.xRot);
                        }
            ...
            if (e instanceof BlazeEntity) {
                if (event.getSpawnReason() == SpawnReason.SPAWNER) {
                    if (Math.random() > 0.85) {
                        ...
                        wildfire.absMoveTo(e.getX(), e.getY(), e.getZ(), e.yRot, e.xRot);

                        world.addFreshEntity(wildfire);
